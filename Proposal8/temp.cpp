...
void readInput(uint8_t* &binReads, char* argv[], char &fileFormatFlag, uint64_t &totalBases, int64_t &numberOfReads, int64_t &maxReadLength, int* &goodRead, uint64_t* &readLocation, uint64_t &currentMemory, uint64_t &peakMemory)
{
	...
	chunkSize = static_cast<uint64_t>(1*readsSize); //Salma
	rewind(inReadfile);
...
void writeOutput(uint8_t * &binReads, char* argv[], char &fileFormatFlag, int* goodRead, uint64_t* &readLocation)
{
	...
	fseek(inReadfile, 0, SEEK_END);
	readsSize = ftell(inReadfile);
	chunkSize = static_cast<uint64_t>(1*readsSize);
	rewind(inReadfile);
	...
	charReads = new unsigned char[chunkSize];
	fread((void *) &charReads[0], sizeof(char), chunkSize, inReadfile); 
	unsigned char * crrReads;
	crrReads = new unsigned char[2*chunkSize];
	unsigned char * crrVals;
	crrVals = new unsigned char[chunkSize];
	uint64_t crrIdx = 0;
	bool hasCorr = false;
	uint64_t lasIdx = 0;
	uint64_t crrValIdx = 0;
	...
	for (i=0; i < chunkSize; i++) 
	{
		...
		{	
			...
			if((i < readsSize) && (charReads[i] != '\r') && (charReads[i] != '\n')){
				hasCorr = true;
				lasIdx = i;
				crrValIdx = 0;
			}
			while ((i < readsSize) && (charReads[i] != '\r') && (charReads[i] != '\n')) 
			{
				for (j=0; j<readLengthInBytes; ++j) 
				{
					...
					for (q=6; q>=offsetCount; q-=2) 
					{
						...
						baseValue &= 3;
						crrVals[crrValIdx]=alphabet[baseValue];
						crrValIdx++;
						i++;
					}
				}
			...
			if(hasCorr){
				...
				while (lasIdx <= i) 
				{
					crrReads[lasIdx + crrIdx] = crrVals[crrValIdx];
					if(charReads[lasIdx] != crrVals[crrValIdx]){
	     				if ((lasIdx+1) < i //current is not the last nuc of read
	     					&&
	     					charReads[lasIdx+1] != crrVals[crrValIdx+1]){ //next is error

	     					if(crrVals[crrValIdx]  == charReads[lasIdx+1]){ //crr of current = next
	                   	      //Delete
	     					  crrIdx -= 1;
		                    } else if (crrVals[crrValIdx+1]  == charReads[lasIdx]){ //current = crr of next
							  //Insert
							  crrReads[lasIdx + crrIdx] = crrVals[crrValIdx];
							  crrReads[lasIdx + crrIdx + 1] = charReads[lasIdx];
							  crrReads[lasIdx + crrIdx + 2] = charReads[lasIdx + 1];
		                   	  crrIdx += 1; 
							  lasIdx++;
							  crrValIdx++;
							}else{
						...
						// }else if((crrValIdx-1) >= 0 //current is not the first nuc of read
						// 	      &&
						// 	     charReads[lasIdx-1] != crrVals[crrValIdx-1]){ //prev is error

		    //                 if(crrVals[crrValIdx]  == charReads[lasIdx-1]){ //crr of current = prev
		    //  				  //Delete
	     //               	      crrIdx -= 1;
		    //                 } else if (crrVals[crrValIdx-1]  == charReads[lasIdx]){ //current = crr of prev
		    //                   //Insert
						// 	  crrReads[lasIdx + crrIdx - 1] = charReads[lasIdx - 1];
						// 	  crrReads[lasIdx + crrIdx] = charReads[crrValIdx];
						// 	  crrReads[lasIdx + crrIdx + 1] = crrVals[crrValIdx];
		    //                	  crrIdx += 1;
		    //                	}else{
						// 	   //Substitute
						// 	   crrReads[lasIdx + crrIdx] = crrVals[crrValIdx];
	     //               		}
	                    } else { //The first/last nuc of the read OR the next/prev is valid
	                    	//Substitute
							crrReads[lasIdx + crrIdx] = crrVals[crrValIdx];
					    }
					}else{
						//No correction
						crrReads[lasIdx + crrIdx] = charReads[lasIdx];
					}
					lasIdx++;
					crrValIdx++;
				}
	...
	fwrite (crrReads , 1 , i+crrIdx-1 , correctedReadfile);
