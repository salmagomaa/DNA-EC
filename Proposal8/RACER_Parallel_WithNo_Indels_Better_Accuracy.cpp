...
void writeOutput(uint8_t * &binReads, char* argv[], char &fileFormatFlag, int* goodRead, uint64_t* &readLocation)
{
    ...
    //Salma 
	unsigned char * crrCharReads;
	crrCharReads = new unsigned char[chunkSize];
	...
	for (k=0, i=0; k < readsSize; ++i, ++k) 
	{
		...
		{	fwrite (crrCharReads , 1 , chunkSize , correctedReadfile);
			fread((void *) &crrCharReads[0], sizeof(char), chunkSize, inReadfile);
			i = 0;
		}
		...
		{	
			...
			while ((i < readsSize) && (crrCharReads[i] != '\r') && (crrCharReads[i] != '\n'))
			{	i++; //read header
				k++;
				if (i == chunkSize)
				{	fwrite (crrCharReads , 1 , chunkSize , correctedReadfile);
			...
			while ((i < readsSize) && (crrCharReads[i] != '\r') && (crrCharReads[i] != '\n')) 
			{
				for (j=0; j<readLengthInBytes; ++j) 
				{
					...
					for (q=6; q>=offsetCount; q-=2) 
					{
						...
						baseValue &= 3;
						crrCharReads[i]=alphabet[baseValue];
						//Salma
						crrCharReads[i]=crrCharReads[i];
						i++;
						k++;
						if (i == chunkSize)
						{	
							//Salma
							// for(uint64_t zs = 0; zs < chunkSize; zs++){
							// 	crrCharReads[i]=crrCharReads[i];
							// }
							fwrite (crrCharReads , 1 , chunkSize , correctedReadfile);
							fread((void *) &crrCharReads[0], sizeof(char), chunkSize, inReadfile);
